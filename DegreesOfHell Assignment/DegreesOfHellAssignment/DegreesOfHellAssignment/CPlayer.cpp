...
CPlayer::CPlayer(const string& name, int year, int motivation, int success, int position, int assignmentCount) :
    mName(name), mYear(year), mMotivation(motivation), mSuccess(success), mPosition(position), mAssignmentCount(0) {}
...
void CPlayer::Move(int steps, const std::vector<CSpace*>& degreeList) {
    ...
    std::cout << GetName() << " lands on " << degreeList[newPosition]->GetName();
    ...
    if (assessmentSpace) {
        ...
        if (!assessmentSpace->IsCompleted()) {
            ...
            if (GetMotivation() >= assessmentSpace->GetMotivationalCost()) {
                ...
                assessmentSpace->CompleteAssessment(GetName()); // Mark the assessment as completed by the current player
                std::cout << std::endl;
                std::cout << "" << std::endl;
                ...
                if (GetMotivation() < 0) { SetMotivation(0); }
            }
            else {
            ...
            while (true) {
                ...
                if (help == "Y" || help == "y") {
                    ...
                    if (helper) {
                        ...
                        std::cout << "" << std::endl;
                        std::cout << "You Both Complete " << assessmentSpace->GetName() << " for Motivation: " << halfMotivationalCost << " and Success:" << halfSuccessScore << " EACH!" << std::endl;
                        std::cout << "" << std::endl;
                        std::cout << GetName() << "'s motivation is " << GetMotivation() << " and success is " << GetSuccess() << std::endl;
                        std::cout << helperName << "'s motivation is " << helper->GetMotivation() << " and success is " << helper->GetSuccess() << std::endl;
                        IncrementAssignmentCount();
                        helper->IncrementAssignmentCount();
                        if (GetMotivation() < 0) { SetMotivation(0); }
                        if (helper->GetMotivation() < 0) { helper->SetMotivation(0); }
                    }
                    else {
                        std::cout << std::endl;
    ...
    else if (CExtracurricular* extracurricularSpace = dynamic_cast<CExtracurricular*>(degreeList[newPosition])) {
        std::cout << std::endl;
        if (!extracurricularSpace->IsCompleted()) {
            // Check if the player has enough motivation to undertake it
            if (GetMotivation() >= extracurricularSpace->GetMotivationalCost()) {
                // Undertake the extracurricular activity
                SetMotivation(GetMotivation() - extracurricularSpace->GetMotivationalCost());
                SetSuccess(GetSuccess() + extracurricularSpace->GetSuccessScore());
                extracurricularSpace->CompleteExtracurricular(GetName()); // Mark the activity as completed by the current player
                std::cout << std::endl;
                std::cout << "" << std::endl;
                std::cout << GetName() << " undertakes " << extracurricularSpace->GetName() << " for Motivation: " << extracurricularSpace->GetMotivationalCost() << " and achieves Success: " << extracurricularSpace->GetSuccessScore() << std::endl;
                std::cout << GetName() << "'s motivation is " << GetMotivation() << " and success is " << GetSuccess() << std::endl;
                IncrementAssignmentCount();
                if (GetMotivation() < 0) { SetMotivation(0); }
            }
                ...
                // If not, output a message indicating insufficient motivation
                std::cout << std::endl;
                std::cout << "Insufficient motivation to undertake the extracurricular activity!" << std::endl;
            }
        }
        else {
            // If extracurricular activity is already completed
            std::cout << std::endl;
            std::cout << "Extracurricular activity: " << extracurricularSpace->GetName() << " is already completed by " << extracurricularSpace->GetCompletedByName() << "!" << std::endl;

            // Ask for help from the player who completed the activity
            std::string help;
            while (true) {
                // Check if completed by the same player as landed on
                if (GetName() == extracurricularSpace->GetCompletedByName()) { break; }

                std::cout << "" << std::endl;
                std::cout << "Do you want help from " << extracurricularSpace->GetCompletedByName() << "? (Y/N): ";
                std::cin >> help;

                if (help == "Y" || help == "y") {
                    // Get the name of the player who completed the activity
                    std::string helperName = extracurricularSpace->GetCompletedByName();
                    ...
                    if (helper) {
                        std::cout << std::endl;
                        // Halve the motivation cost and success score
                        int halfMotivationalCost = extracurricularSpace->GetMotivationalCost() / 2;
                        int halfSuccessScore = extracurricularSpace->GetSuccessScore() / 2;
                        ...
                        std::cout << "" << std::endl;
                        std::cout << "You accepted help from " << helperName << std::endl;
                        std::cout << "" << std::endl;
                        std::cout << "Both of you complete " << extracurricularSpace->GetName()  << std::endl;
                        std::cout << "" << std::endl;
                        std::cout << GetName() << "'s motivation is " << GetMotivation() << " and success is " << GetSuccess() << std::endl;
                        std::cout << helperName << "'s motivation is " << helper->GetMotivation() << " and success is " << helper->GetSuccess() << std::endl;
                        IncrementAssignmentCount();
                    ...
                    std::cout << "" << std::endl;
                    std::cout << "You declined help from " << extracurricularSpace->GetCompletedByName() << ". Your game continues." << std::endl;
                    break; // Exit the loop after valid input
                }
                else {
                    std::cout << "Invalid input. Please enter 'Y' or 'N'." << std::endl;
        ...
        // If the new position is not an assessment or extracurricular space, simply move the player
        SetPosition(newPosition);
        //std::cout << GetName() << "'s motivation is " << GetMotivation() << " and success is " << GetSuccess() << std::endl;
    }
}
...
int CPlayer::GetAssignmentCount() {
    return mAssignmentCount;
}

// Function to increment assignment count
void CPlayer::IncrementAssignmentCount() {
    mAssignmentCount++;
}

// Function to reset assignment count
void CPlayer::ResetAssignmentCount() {
    mAssignmentCount = 0;
}